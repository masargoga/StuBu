...
- **Language selector (UC-016):** a `Select` with a flag image (`.flag`, 1.5rem x 1rem, drawn from `icons/flags/*.svg`) and the language name written in that language; in the header before the user's avatar and in the corner of the login card. On phones (up to 640px) only the flag shows in the closed field (`4.75rem` wide) so that the long French and Spanish texts of the header still fit.
- **Regions (UC-017):** the holidays page has a toolbar with the region selector, the year selector and the quiet button "Manage regions"; the regions dialog lists each region with its number of employees and holidays and Rename/Delete as text buttons. The note "Public holidays: <region>" (`.holiday-region-note`, secondary text colour) sits above the month on every timesheet view. The list pages of the administrator (employees, holidays) turn into cards below 1500px and 900px; the wide grid of the employee list applies only from 1501px.
