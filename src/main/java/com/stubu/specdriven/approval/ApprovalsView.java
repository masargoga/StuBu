...
public class ApprovalsView extends VerticalLayout implements HasDynamicTitle, LocaleChangeObserver {
    ...
    private static final String FLASH_ATTRIBUTE = "approvals.flash";
    private static final Logger log = LoggerFactory.getLogger(ApprovalsView.class);

    /** A message to show on the list after a decision was made on the review page. */
    public record Flash(String key, boolean error) implements Serializable {
    }

    /** Shows the message on the list the next time it is opened. */
    public static void flash(String key, boolean error) {
        VaadinSession session = VaadinSession.getCurrent();
        if (session != null) {
            session.setAttribute(FLASH_ATTRIBUTE, new Flash(key, error));
        }
    }
    ...
    public ApprovalsView(AuthenticationContext authenticationContext, TimesheetReviewService service,
        ...
        VaadinSession session = VaadinSession.getCurrent();
        if (session != null) {
            message = (Flash) session.getAttribute(FLASH_ATTRIBUTE);
