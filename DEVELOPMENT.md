...
Java 25 is required (`JAVA_HOME`). To run the application with sample employees and a mock identity provider (no real Microsoft/Google account needed):

```bash
./mvnw spring-boot:test-run -Dspring-boot.run.main-class=com.stubu.specdriven.DevApplication
```
