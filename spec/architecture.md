...
- **UI texts:** all user-facing strings live in `src/main/resources/vaadin-i18n/translations*.properties` (English default, German, Spanish, French) and are applied in `LocaleChangeObserver.localeChange`, because a component's locale is only known once it is attached.
...
- **Public holidays:** `PublicHolidayService` (holiday package) adds, changes and deletes the holidays of a region for an active administrator (`AdminAccess`), validates date and name itself (the date is unique per region), and stores each change with its audit entry in one transaction. `PublicHolidayView` (`/admin/holidays`, ADMIN only) is the page: a region selector, the holidays of the chosen region, and `RegionManagementDialog` to add, rename and delete regions.
- **Regions (UC-017):** `Region` (region package, table `region`, migration `V9`) is kept by `RegionService` (add, rename with optimistic locking, delete only when no employee and no holiday uses it and it is not the last region; every change audited as entity `Region`). `Employee.regionId` and `PublicHoliday.regionId` are required foreign keys; the migration creates the region "Default" (id 1, `Region.DEFAULT_ID`) for existing data. `MonthlyTimesheetService` looks up the region of the employee whose timesheet is built and marks that region's holidays, so the employee, the reviewing manager (UC-007) and the manager or administrator looking at the timesheet (UC-009, UC-010, UC-014) all see the same holidays; `RegionNote` shows the region name above the month.
...
- **Application language (UC-016):** `AppLanguage` lists the four languages (code, native name, flag). `LanguageInitListener` chooses the language of every new UI: the signed-in employee's stored language (table `employee_setting`, `EmployeeSettingsService`), else the cookie `stubu-language`, else the browser's `Accept-Language` if supported, else English (`LanguageResolver`); it also sets the `lang` attribute of the html element (WCAG 3.1.1). `LanguageSelector` (a `Select` with flag and name) is in the header and on the login card; it switches the UI locale at once, sets the cookie and saves the choice without an audit entry. `LoginSuccessHandler` stores a language chosen on the login page when the employee has none yet. Emails are sent in the recipient's language (`EmailNotificationService`), falling back to `stubu.notifications.locale`. Flags are SVG images in `META-INF/resources/icons/flags` because emoji flags do not show on Windows.
