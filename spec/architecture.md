...
- **Employee overview (administrators):** `EmployeeOverviewService` (admin package) filters, searches and sorts the list of `EmployeeAdminService.list` (which adds the last login from the audit log) and returns one employee with all timesheets; it is read-only. `EmployeeManagementView` shows the list and `EmployeeDetailView` (`/admin/employees/details/{id}`, ADMIN only) the details.
