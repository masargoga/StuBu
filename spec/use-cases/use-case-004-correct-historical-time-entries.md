# UC-004: Correct Historical Time Entries

---

**Goal:** As an employee, I want to correct or delete previous time entries so that my records are accurate.

**Status:** Implemented
**Date:** 2024-01-15

> A use case cannot be marked as **Implemented** unless all criteria in the use case implementation workflow are fulfilled.

> **Revision:** Times are absolute instants (see `spec.md` section 8 and the data model), and a period may cross midnight. The edit form therefore has a read-only check-in date (the date of an entry cannot change) and a date plus time for the check-out. Deleting removes the entry permanently; the audit log keeps its times, so no soft-delete flag is needed. The delete dialog has an optional reason field, so the audit log can record it. The timesheet status that AF-3 depends on comes from a minimal timesheet record (employee, month, status); a month without a record counts as a draft. UC-005 and UC-006 extend the timesheet.

---

## Actors

- **Primary actor:** Employee (authenticated)

---

## Preconditions

- User is authenticated and has the EMPLOYEE role
- TimeEntry exists that the employee owns
- The TimeEntry's parent Timesheet (if any) is in DRAFT status (not yet submitted)

---

## Trigger

Employee clicks "Edit" or "Delete" on a time entry in the timeline or daily view.

---

## Main Flow

### Edit Flow

1. Employee clicks "Edit" on a time entry.
2. System displays an edit form with:
   - Check-in date (read-only, the date of an entry cannot be changed)
   - Check-in time (editable)
   - Check-out date and time (editable; empty while the entry is still open)
   - Reason for the correction (optional text field)
3. Employee updates check-in and/or check-out times.
4. Employee optionally enters a reason for the correction.
5. Employee clicks "Save".
6. System validates:
   - Check-out must be after check-in (or empty if the entry is still open)
   - Neither time may be in the future
   - The entry must not overlap another work period of the employee
7. System updates the TimeEntry in the database. Times the employee did not touch keep their recorded seconds.
8. System creates an audit log entry with action UPDATE, capturing old and new values and the reason.
9. System recalculates day totals (the daily view is reloaded).
10. System displays success message: "Time entry updated."
11. System returns to the timeline view.

### Delete Flow

12. Employee clicks "Delete" on a time entry.
13. System displays a confirmation dialog: "Are you sure you want to delete this time entry? This cannot be undone." with an optional reason field.
14. Employee confirms deletion.
15. System deletes the TimeEntry from the database.
16. System creates an audit log entry with action DELETE, capturing the deleted times and the reason (if provided).
17. System recalculates day totals.
18. System displays success message: "Time entry deleted."
19. System returns to the timeline view.

---

## Alternative Flows

### AF-1: Validation Fails (Check-Out Before Check-In)

**Branches from:** Main Flow step 6 (Edit validation)
**Condition:** Check-out time is not after check-in time

1. System displays error: "Check-out time must be after check-in time."
2. User remains in the edit form.
3. User corrects the times and resubmits.
4. Returns to Main Flow step 6.

### AF-2: Validation Fails (Future Date)

**Branches from:** Main Flow step 6 (Edit validation)
**Condition:** Check-in or check-out time is in the future

1. System displays error: "You cannot log time in the future."
2. User remains in the edit form.
3. User corrects the times and resubmits.
4. Returns to Main Flow step 6.

### AF-3: Timesheet Already Submitted

**Branches from:** Preconditions
**Condition:** The TimeEntry belongs to a Timesheet with status SUBMITTED, APPROVED, or REJECTED

1. System disables "Edit" and "Delete" buttons for that entry.
2. System displays a message with the entry: "This entry cannot be edited because the timesheet has been submitted. Wait for approval or rejection before making changes."
3. If the timesheet becomes submitted while a dialog is open, saving or deleting is refused with the same message.
4. Use case ends.

### AF-4: Database Error

**Branches from:** Main Flow step 7 or 15
**Condition:** Database operation fails

1. System displays error: "Unable to save changes. Please try again."
2. User remains in the edit form (for edit) or in the delete dialog (for delete), which is the retry option.
3. User can retry.
4. Use case ends without state change.

### AF-5: Other Validation and Concurrency Errors

**Branches from:** Main Flow step 6, 7 or 15
**Condition:** The input is incomplete or conflicts with the employee's other data

1. Missing check-in time: "Please enter the check-in time."
2. Check-out date or time entered without the other, or removed from a completed entry: "Please enter both the date and the time of the check-out." / "Please enter the check-out date and time."
3. Overlap with another work period: "This period overlaps a work period you already recorded."
4. The entry was deleted in the meantime: "This time entry no longer exists." and the day is reloaded.
5. In all validation cases nothing is changed and the employee stays in the dialog.

---

## Postconditions

- **On success (Edit):**
  - TimeEntry is updated with new check-in/check-out times
  - Audit log captures old and new values
  - Timeline is refreshed to show updated entry
  - Day totals are recalculated

- **On success (Delete):**
  - TimeEntry is removed from timeline
  - Audit log captures deletion
  - Day totals are recalculated

- **On failure:**
  - No changes are made to TimeEntry
  - Error message is displayed
  - User can retry or cancel

---

## Business Rules

| ID | Rule |
|----|------|
| BR-01 | Employees can only edit/delete their own time entries |
| BR-02 | Corrections are only allowed if the parent Timesheet is in DRAFT status (a month without a timesheet record counts as DRAFT) |
| BR-03 | Check-out time must be after check-in time (or empty if active); a completed entry cannot be reopened |
| BR-04 | Times cannot be in the future; the check-in date cannot be changed; work periods must not overlap |
| BR-05 | All corrections must be logged to AuditLog with old/new values and reason |
| BR-06 | Managers and admins can also edit/delete employee entries (see separate use case) |

---

## Tests

- [x] Main Flow Edit covered (steps 1–11)
- [x] Main Flow Delete covered (steps 12–19)
- [x] AF-1 (Validation: Check-Out Before Check-In) covered
- [x] AF-2 (Validation: Future Date) covered
- [x] AF-3 (Timesheet Submitted) covered
- [x] AF-4 (Database Error) covered
- [x] AF-5 (Other validation and concurrency errors) covered
- [x] BR-01–BR-05 covered (BR-06 belongs to the manager and admin use cases)

---

## UI Surface

- **Timeline / Daily View:** Each time entry has "Edit" and "Delete" actions (disabled, with an explanation in the row, if the timesheet is submitted).
- **Edit Form:** Modal showing the check-in date (read-only), check-in time, check-out date and time, and reason text field.
- **Confirmation Dialog:** For delete action, confirming the destructive operation, with an optional reason.
- **Success / Error Messages:** Clear feedback on edit/delete results.

| Page | Access |
|------|--------|
| Edit Time Entry (dialog on the home page) | Authenticated Employee (own entries only) |
