...
> **Revision (UC-017):** Holidays belong to a region. The page has a region selector (the first region by name is chosen) and shows the holidays of that region; "Add holiday" has a region selection in the form (the chosen region is preselected) and afterwards the page shows the region the holiday was added to; when editing, the region is shown read-only (a holiday cannot move to another region). BR-02 now reads: each date can have at most one holiday per region. Audit entries carry the region name.
